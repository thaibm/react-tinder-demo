...
const draggableTouchmove = (coordinates, element, offset, originalLocation) => {
...
const TinderCard = React.forwardRef(({ flickOnSwipe = true, children, onSwipe, onCardLeftScreen, className, preventSwipe = [] }, parentRef) => {
  ...
  const ref = React.useCallback((element) => {
    ...
    element.addEventListener(('touchmove'), (ev) => {
      ev.preventDefault()
      const newLocation = draggableTouchmove(touchCoordinatesFromEvent(ev), element, offset, originalLocation)
    ...
    element.addEventListener(('mousemove'), (ev) => {
      ev.preventDefault()
      if (mouseIsClicked) {
        const newLocation = draggableTouchmove(mouseCoordinatesFromEvent(ev), element, offset, originalLocation)
        speed = calcSpeed(lastLocation, newLocation)
        lastLocation = newLocation
      }
    })
